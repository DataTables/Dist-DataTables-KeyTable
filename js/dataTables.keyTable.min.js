...
function(c){f(c.nTable).triggerHandler(a,b)})},_focus:function(a,b,c,d){var e=this,l=this.s.dt,g=l.page.info(),i=this.s.lastFocus;d||(d=null);if(this.s.enable){if("number"!==typeof a){if(!a.any())return;var j=a.index(),b=j.column,a=l.rows({filter:"applied",order:"applied"}).indexes().indexOf(j.row);if(0>a)return;g.serverSide&&(a+=g.start)}if(-1!==g.length&&(a<g.start||a>=g.start+g.length))this.s.focusDraw=!0,this.s.waitingForDraw=!0,l.one("draw",function(){e.s.focusDraw=!1;e.s.waitingForDraw=!1;e._focus(a,
